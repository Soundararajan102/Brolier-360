...
describe('hashApiKey', () => {
  it('is deterministic', () => {
    expect(hashApiKey('durocrm_live_abc')).toBe(hashApiKey('durocrm_live_abc'));
  });

  it('differs for different inputs', () => {
    expect(hashApiKey('durocrm_live_abc')).not.toBe(hashApiKey('durocrm_live_abd'));
  });
});
...
describe('timingSafeHexEqual', () => {
  it('is true for identical digests', () => {
    const h = hashApiKey('durocrm_live_xyz');

...
The public API lets you drive your Duro CRM instance from your own
...
```
Authorization: Bearer durocrm_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
3. Copy the key. **The full key is shown exactly once.** Duro CRM
...
curl https://your-crm.example.com/api/v1/me \
  -H "Authorization: Bearer durocrm_live_xxx"
```
...
Rather than polling, register an endpoint and Duro CRM will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; Duro CRM keeps only an encrypted copy.
...
curl -X POST https://your-crm.example.com/api/v1/webhooks \
  -H "Authorization: Bearer durocrm_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://example.com/hooks/durocrm", "events": ["message.received"] }'
...
Headers: `X-DuroCRM-Event`, `X-DuroCRM-Webhook-Id`, and `X-DuroCRM-Signature`.

### Verifying the signature

`X-DuroCRM-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages Duro CRM stores (inbox + API sends), not broadcast-only
...
[#245](https://github.com/Soundararajan102/Brolier-360/issues/245). Future ideas
